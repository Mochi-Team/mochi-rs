// Most, if not all, of this source code was copied/modified from Aidoku-RS
// https://github.com/Aidoku/aidoku-rs/blob/main/crates/imports/src/html.rs

extern crate alloc;

use crate::imports::core::Kind;
use core::fmt::Display;

use alloc::string::String;

use super::core::{PtrRef, ArrayRef, ptr_kind};

use super::error::{Result, MochiError, NodeError};
...
extern "C" {
    ...
    fn scraper_parse_fragment(string: *const u8, len: usize) -> HostPtr;
    ...
    #[link_name = "select"]
    fn scraper_select(ptr: HostPtr, selector: *const u8, selector_len: usize) -> i32;
    #[link_name = "attr"]
    fn scraper_attr(ptr: HostPtr, selector: *const u8, selector_len: usize) -> i32;

    #[link_name = "set_text"]
    fn scraper_set_text(ptr: HostPtr, text: *const u8, text_len: usize) -> i32;
    #[link_name = "set_html"]
    fn scraper_set_html(ptr: HostPtr, html: *const u8, html_len: usize) -> i32;
    #[link_name = "prepend"]
    fn scraper_prepend(ptr: HostPtr, html: *const u8, html_len: usize) -> i32;
    #[link_name = "append"]
    fn scraper_append(ptr: HostPtr, html: *const u8, html_len: usize) -> i32;

    #[link_name = "first"]
    fn scraper_first(ptr: HostPtr) -> HostPtr;
    #[link_name = "last"]
    fn scraper_last(ptr: HostPtr) -> HostPtr;
    #[link_name = "next"]
    fn scraper_next(ptr: HostPtr) -> HostPtr;
    #[link_name = "previous"]
    fn scraper_previous(ptr: HostPtr) -> HostPtr;

    #[link_name = "base_uri"]
    fn scraper_base_uri(ptr: HostPtr) -> HostPtr;
    #[link_name = "body"]
    fn scraper_body(ptr: HostPtr) -> HostPtr;
    #[link_name = "text"]
    fn scraper_text(ptr: HostPtr) -> HostPtr;
    #[link_name = "untrimmed_text"]
    fn scraper_untrimmed_text(ptr: HostPtr) -> HostPtr;
    #[link_name = "own_text"]
    fn scraper_own_text(ptr: HostPtr) -> HostPtr;
    #[link_name = "data"]
    fn scraper_data(ptr: HostPtr) -> HostPtr;
    #[link_name = "array"]
    fn scraper_array(ptr: HostPtr) -> HostPtr;
    #[link_name = "html"]
    fn scraper_html(ptr: HostPtr) -> HostPtr;
    #[link_name = "outer_html"]
    fn scraper_outer_html(ptr: HostPtr) -> HostPtr;

    #[link_name = "escape"]
    fn scraper_escape(ptr: HostPtr) -> HostPtr;
    #[link_name = "unescape"]
    fn scraper_unescape(ptr: HostPtr) -> HostPtr;

    #[link_name = "id"]
    fn scraper_id(ptr: HostPtr) -> i32;
    #[link_name = "tag_name"]
    fn scraper_tag_name(ptr: HostPtr) -> i32;
    #[link_name = "class_name"]
    fn scraper_class_name(ptr: HostPtr) -> i32;
    #[link_name = "has_class"]
    fn scraper_has_class(ptr: HostPtr, class_name: *const u8, class_length: usize) -> bool;
    #[link_name = "has_attr"]
    fn scraper_has_attr(ptr: HostPtr, attr_name: *const u8, attr_length: usize) -> bool;
}
...
pub fn escape_html_entities<T: AsRef<str>>(text: T) -> String {
    let ptr_ref = PtrRef::from(text.as_ref());
    let host_id = unsafe { 
        scraper_escape(ptr_ref.pointer()) 
    };
    PtrRef::new(host_id).as_string().unwrap_or_default()
}
...
pub fn unescape_html_entities<T: AsRef<str>>(text: T) -> String {
    let ptr_ref = PtrRef::from(text.as_ref());
    let host_id = unsafe { 
        scraper_unescape(ptr_ref.pointer()) 
    };
    PtrRef::new(host_id).as_string().unwrap_or_default()
}
...
#[derive(Debug)]
pub struct Node(pub(crate) HostPtr);
...
impl Node {
    ...
    pub fn new<T: AsRef<[u8]>>(buf: T) -> Result<Self> {
        let buf = buf.as_ref();
        let host_id = unsafe { scraper_parse(buf.as_ptr(), buf.len()) };
        if host_id >= 0 {
            Ok(Self(host_id))
        } else {
            Err(MochiError::from(NodeError::ParserError))
        }
    }
    ...
    pub fn new_fragment<T: AsRef<[u8]>>(buf: T) -> Result<Self> {
        let buf = buf.as_ref();
        let host_id = unsafe { scraper_parse_fragment(buf.as_ptr(), buf.len()) };
        if host_id >= 0 {
            Ok(Self(host_id))
        } else {
            Err(MochiError::from(NodeError::ParserError))
    ...
    /// Get an instance from a [PtrRef](crate::PtrRef)
    ///
    /// # Safety
    /// Ensure that this Ptr is of [Kind::Node](crate::Kind) before
    /// converting.
    #[inline]
    pub unsafe fn from(ptr: HostPtr) -> Self {
        Self(ptr)
    }
    ...
    pub fn select<T: AsRef<str>>(&self, selector: T) -> Self {
        ...
        let host_id = unsafe { scraper_select(self.0, selector.as_ptr(), selector.len()) };
        Self(host_id)
    }
    ...
    pub fn attr<T: AsRef<str>>(&self, attr: T) -> String {
        ...
        let host_id = unsafe { scraper_attr(self.0, attr.as_ptr(), attr.len()) };
        PtrRef::new(host_id).as_string().unwrap_or_default()
    }
    ...
    pub fn set_html<T: AsRef<str>>(&mut self, html: T) -> Result<()> {
        ...
        match unsafe { scraper_set_html(self.0, html.as_ptr(), html.len()) } {
            0 => Ok(()),
            _ => Err(MochiError::from(NodeError::ModifyError)),
        }
    }
    ...
    pub fn next(&self) -> Option<Node> {
        let ptr = unsafe { scraper_next(self.0) };
        match unsafe { ptr_kind(ptr) } {
            Kind::Node => Some(Node(ptr)),
            _ => None,
        }
    }
    ...
    pub fn previous(&self) -> Option<Node> {
        let ptr = unsafe { scraper_previous(self.0) };
        match unsafe { ptr_kind(ptr) } {
            Kind::Node => Some(Node(ptr)),
            _ => None,
        }
    ...
    pub fn base_uri(&self) -> String {
        let ptr = unsafe { scraper_base_uri(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }

    /// Get the document's `body` element.
    pub fn body(&self) -> String {
        let ptr = unsafe { scraper_body(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }
    ...
    pub fn text(&self) -> String {
        let ptr = unsafe { scraper_text(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }
    ...
    pub fn untrimmed_text(&self) -> String {
        let ptr = unsafe { scraper_untrimmed_text(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }
    ...
    pub fn own_text(&self) -> String {
        let ptr = unsafe { scraper_own_text(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }
    ...
    pub fn data(&self) -> String {
        let ptr = unsafe { scraper_data(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }
    ...
    pub fn html(&self) -> String {
        let ptr = unsafe { scraper_html(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }
    ...
    pub fn outer_html(&self) -> String {
        let ptr = unsafe { scraper_outer_html(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }
    ...
    pub fn escape(&self) -> String {
        let ptr: i32 = unsafe { scraper_escape(self.0) };
        PtrRef::new(ptr).as_string().unwrap_or_default()
    }
    ...
    pub fn unescape(&self) -> String {
        let host_id: i32 = unsafe { scraper_unescape(self.0) };
        PtrRef::new(host_id).as_string().unwrap_or_default()
    }

    /// Get the `id` attribute of this element.
    pub fn id(&self) -> String {
        let host_id = unsafe { scraper_id(self.0) };
        PtrRef::new(host_id).as_string().unwrap_or_default()
    }
    ...
    pub fn tag_name(&self) -> String {
        let host_id = unsafe { scraper_tag_name(self.0) };
        PtrRef::new(host_id).as_string().unwrap_or_default()
    }
    ...
    pub fn class_name(&self) -> String {
        let host_id = unsafe { scraper_class_name(self.0) };
        PtrRef::new(host_id).as_string().unwrap_or_default()
    }
...
impl Clone for Node {
    fn clone(&self) -> Self {
        let ptr: HostPtr = unsafe { copy(self.0) };
        Self(ptr)
    }
}